...
import { ForgotPasswordRequest, ForgotPasswordResponse, JwtPayload, LoginRequest, LoginResponse, PasswordResetTokenPayload, ResetPasswordRequest } from './dto/auth.dto';
...
export class AuthService {
  ...
  async login(loginRequest: LoginRequest): Promise<LoginResponse> {
    try {
      ...
      const passwordIsMatched = user.password === HashUtils.hashPassword(loginRequest.password, user.salt);
      if (!passwordIsMatched) { // Login fail with invalid password
        return new LoginResponse({
          responseMessage: new ResponseMessage({
            status: ResponseStatus.Fail,
            messageCode: MessageCode.FAIL,
  ...
  async forgotPassword(forgotPasswordRequest: ForgotPasswordRequest): Promise<ForgotPasswordResponse> {
    try {
      const user = await this.usersService.findOneByEmail(forgotPasswordRequest.email);
      if (!user) {
      ...
      const result = await this.redisClient.hGet(REDIS_KEY_FORGOT_PASSWORD, keyCacheRequest);
      let isWaiting: boolean = false;
      let diffTime: number = 0;
      const FORGOT_PASSWORD_WAIT_TIME_IN_SECONDS = +this.configService.get('FORGOT_PASSWORD_WAIT_TIME_IN_SECONDS') || 60;

      if (result !== null) {
        diffTime = CommonUtils.diffTotalSeconds(new Date(Date.now()), new Date(+result));
      ...
      await this.redisClient.hSet(REDIS_KEY_FORGOT_PASSWORD, keyCacheRequest, Date.now().toString());
      const payload = new PasswordResetTokenPayload({
        userId: user.id,
        expire: Date.now() + (+this.configService.get('PASSWORD_RESET_TOKEN_LIFE_TIME_IN_SECONDS') * 1000),
      });
  ...
  async resetPassword(request: ResetPasswordRequest): Promise<ResponseMessage> {
    try {
      const decrypted: PasswordResetTokenPayload = HashUtils.decryptCode(request.token);
      ...
      const userToken: UserTokenEntity = await this.userTokenService.findUserResetPasswordToken(userId, request.token);

      if (!userToken) {
        return new ResponseMessage({
          status: ResponseStatus.Fail,
          messageCode: MessageCode.ACCOUNT_RESET_PASSWORD_INVALID_RESET_TOKEN_MESSAGE,
        });
      }

      const user: UserEntity = await this.usersService.findOneById(userId);
      const isMatchedPassword = HashUtils.hashPassword(request.password, user.salt) === user.password;

      if (isMatchedPassword) {
        return new ResponseMessage({
          status: ResponseStatus.Fail,
          messageCode: MessageCode.ACCOUNT_RESET_PASSWORD_INVALID_NEW_PASSWORD_MESSAGE,
        });
      }
